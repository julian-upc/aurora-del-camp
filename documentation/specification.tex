...
\item[Activities common to all crops]  $ti$~for tilling, $rv$~for
  ...
  $\ell\in L$ and $i\in R$, which we write as $ti_{\ell, i}$ and which
  means ``tilling the lot $\ell$ for the $i$-th time''. Thus,
...
The estimates $|L|=30$, $|R|=5$, $|C|=40$ yield
\[
...
in~$A$; thus, $|E|=2|A|$. Following \cite{artigues-etal11}, we
