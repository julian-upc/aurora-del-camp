...
activity, such as the duration or the yield, may depend on the start time. We let
$W$~denote the set of all useful start time windows $\omega = \omega_{\rm start}$, i.e.,
all intervals contained in the planning horizon of the problem that are eligible for
starting a task.

Gilad breaks these two types of activities down as follows:
...
\item[Activities common to all crops] $ti$~for tilling, $rv$~for rotovating, $gm$~for
  ...
  $\ell$ starting in the window~$\omega$''.
...
\item[Activities specific to a crop] $by$ for buying seeds, $ss$ for soaking seeds, $cs$
  ...
  $\ell\in L$, starting during the window~$\omega$''.
\end{mydesc}
...
\]
with $w_0<w_1<w_2<w_3<w_4<w_5$
is a chain consisting of planting, weeding, and harvesting a crop $c$ in a lot~$\ell$ that
needs $w_4-w_1$~weeks to grow. The directed graph~$H$ records that in this particular
chain, \emph{growing} and \emph{weeding} may be carried out in parallel, but both these
activities come after \emph{planting} and before \emph{harvesting}. In general, the set of
all chains is denoted by~$K$, and if the first activity in a chain~$\chi$ is executed, all
the others must be executed too. Some conditions that may be expressed using this language
are the following.

\begin{itemize}
\item Tomatoes planted in week 25 can be harvested in week 26, but tomatoes planted in
  week 40 can be harvested only in week 45.
\item \emph{``A head of lettuce planted in summer must be harvested the week after it is planted,
  but if it is planted in winter, it can stay in the ground for up to two months.''} This
can be modeled as
\[
  ...
\]
